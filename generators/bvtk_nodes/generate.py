...
class VTK{{C.NAME}}(Node, {{BASE}}):
    ...
    {% endfor %}
    b_properties: bpy.props.BoolVectorProperty(name="", size={{C.NP}}, get={{BASE}}.get_b, set={{BASE}}.set_b) #type: ignore
...
filenames = {
    key: (call_dir if len(call_dir) > 0 else ".") + "/../../addons/braas_hpc_pynari_composer/bvtk_nodes/" + val
